...
export const roomHandlers = [
  ...
  http.get('/api/v1/training-runs', ({ request }) => {
    const url = new URL(request.url)
    const roomId = url.searchParams.get('room_id')

    if (roomId && roomId !== fixtureRoom.id) {
  ...
  http.get('/api/v1/rooms/:roomId/invites', ({ params }) => {
    ...
    return HttpResponse.json(
      invites.filter((i) => !i.is_revoked),
    )
  }),

  http.post(
    '/api/v1/rooms/:roomId/invites',
  ...
  http.delete(
    '/api/v1/rooms/:roomId/invites/:inviteId',
    ({ params }) => {
      const invite = invites.find(
        (i) => i.id === params.inviteId,
      )
  ...
  http.post('/api/v1/rooms/join', async ({ request }) => {
    ...
    if (body.invite_code === 'DUPEJOIN') {
      return HttpResponse.json(
        { detail: 'User has already joined this room' },
        { status: 409 },
      )
    }
  ...
  http.post(
    '/api/v1/rooms/:roomId/nodes/:peerId/revoke',
    ({ params }) => {
      if (params.roomId !== fixtureRoom.id) {
        return HttpResponse.json(
          { detail: 'Room not found' },

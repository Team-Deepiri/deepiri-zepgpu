...
export default function TaskDetail() {
  ...
  const isRoomDispatch =
    task?.dispatch_mode && task.dispatch_mode !== 'local'
  ...
  if (!task) {
    return (
      <div className="text-center py-12">
        ...
        <Link
          to="/tasks"
          className="text-zepgpu-400 hover:text-zepgpu-300 mt-2 inline-block"
        >
          Back to tasks
        </Link>
      </div>
  ...
  const backHref = task.room_id
    ? `/rooms/${task.room_id}`
    : '/tasks'

  const backLabel = task.room_id
    ? 'Back to room'
    : 'Back to tasks'
    ...
    <div className="space-y-6">
      ...
      {task.error && (
        <div className="bg-red-500/10 border border-red-500/50 rounded-xl p-6">
          ...
          <pre className="text-sm text-red-300 whitespace-pre-wrap break-words max-h-96 overflow-auto">
            {task.error}
          </pre>
        </div>
      )}
              ...
              <div className="space-y-3 text-sm">
                <p className="text-slate-400">
                  ...
                  <span className="text-white">
                    {remoteResultQuery.data.assignment_status}
                  </span>
                </p>
                ...
                {remoteResultQuery.data.result_size_bytes != null && (
                  <p className="text-slate-400">
                    Size:{' '}
                    {remoteResultQuery.data.result_size_bytes.toLocaleString()}{' '}
                    bytes
                  </p>
                )}

                {Object.keys(
                  remoteResultQuery.data.result_metadata ?? {},
                ).length > 0 && (
                  <SafeJsonViewer
                    value={remoteResultQuery.data.result_metadata}
                    maxInlineChars={25_000}
                    className="text-slate-300 bg-slate-900/60 rounded-lg p-4 border border-slate-700"
                  />
                )}

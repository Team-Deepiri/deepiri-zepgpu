...
class Peer(UUIDMixin, TimestampMixin, Base):
    ...
    vpn_network_id: Mapped[str] = mapped_column(
        UUID(as_uuid=True), ForeignKey("vpn_networks.id", ondelete="CASCADE"), nullable=False, index=True
    )

    wireguard_public_key: Mapped[str] = mapped_column(String(255), unique=True, nullable=False, index=True)
...
class GpuShare(UUIDMixin, TimestampMixin, Base):
    ...
    peer_id: Mapped[str] = mapped_column(
        UUID(as_uuid=True), ForeignKey("vpn_peers.id", ondelete="CASCADE"), nullable=False, index=True
    )
    vpn_network_id: Mapped[str] = mapped_column(
        UUID(as_uuid=True), ForeignKey("vpn_networks.id", ondelete="CASCADE"), nullable=False, index=True
    )
...
class GpuShareQuota(UUIDMixin, TimestampMixin, Base):
    ...
    peer_id: Mapped[str] = mapped_column(
        UUID(as_uuid=True), ForeignKey("vpn_peers.id", ondelete="CASCADE"), unique=True, nullable=False
    )

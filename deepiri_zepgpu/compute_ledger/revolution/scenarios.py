...
async def run_credit_economy_scenario(db: AsyncSession) -> EconomyScenarioResult:
    ...
    result.steps.append(
        ScenarioStep("dual_chain_genesis", True, "isolated VPN chains initialized", {
            "chain_a": a.chain_id,
            "chain_b": b.chain_id,
        })
    )
    ...
    result.steps.append(
        ScenarioStep(
            ...
            {"height": earn["block"]["height"] if earn.get("block") else None, "gpu_seconds": 100.0},
        )
    )

    # Raise quorum and require second approval
    ...
    quorum_unfinalized = block is not None and block.get("finalized") is False
    if quorum_unfinalized:
        sig = sign_message(priv2, block["hash"])
        finalized = await a.approve_block(block["hash"], validator_public_key=pub2, signature=sig)
    ...
    bridge_ok = (
        ...
        and abs(a_bal.get("provider-nova", {}).get("net_seconds", 0) - 85.0) < 1e-9  # 125 - 40
    ...
    # Inclusion proof for a sealed tx
    ...
    # Attack: forged approve on finalized tip (should fail)
    ...
    # Concurrent attestations on B
    async def _earn(i: int) -> dict[str, Any]:
        svc = LedgerService(db, network_id=net_b)
        return await svc.record_job_completed(
    ...
    # Sequential under shared session is safer; still stress multiple seals
    concurrent_ok = True
    for i in range(5):
        out = await _earn(i)
    ...
    result.steps.append(
        ScenarioStep(
            ...
            {"tip_height": b_final.get("tip_height"), "balances": len(b_final.get("balances") or [])},
        )
    )
...
async def run_db_adversary_probes(db: AsyncSession) -> list[ScenarioStep]:
    ...
    settings.ledger.chain_id = chain
    settings.ledger.quorum_threshold = 1
    settings.ledger.auto_seal = True
    settings.ledger.extra_validator_private_keys = ""
    ...
    # Double-seal while unfinalized tip exists
    settings.ledger.quorum_threshold = 2
    priv2, pub2 = generate_keypair()
    ...
    # cleanup finalize so later tests ok
    if first.get("block") and not first["block"].get("finalized"):
        sig = sign_message(priv2, first["block"]["hash"])
        await svc.approve_block(first["block"]["hash"], validator_public_key=pub2, signature=sig)
    settings.ledger.quorum_threshold = 1
    return steps

...
Token issuance lives in ``deepiri_zepgpu.rooms.provider_tokens`` so repositories
do not depend on the API layer.
"""
...
from deepiri_zepgpu.rooms.provider_tokens import (
    issue_provider_token as _issue_provider_token,
)
...
async def issue_provider_token(
    ...
    *,
    ...
    """HTTP-facing wrapper around room-scoped token issuance."""

    try:
        return await _issue_provider_token(
            peer_repo,
            peer,
            rotate=rotate,
            provider_mode=provider_mode,
        )
    except ProviderRevokedError as exc:
